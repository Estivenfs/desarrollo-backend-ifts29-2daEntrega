...
class Turno {
  ...
  async validate() {
    ...
    if (
      ...
      this.isValidTime(this.HoraInicio)
    ) {
      ...
      if (fechaHoraTurno <= ahora) {
        errors.push(
          "La fecha y hora del turno deben ser posteriores al momento actual"
        );
      }
    }
    ...
    if (!this.IdPaciente) {
      ...
      const paciente = await DatabaseService.getById(
        "pacientes",
        this.IdPaciente
      );
      if (!paciente) {
  ...
  static async checkConflicto(
    ...
    return turnos.some((turno) => {
      ...
      const hayConflicto =
        horaInicio < turno.HoraFin && turno.HoraInicio < horaFin;
  ...
  static async getTurnosCompletos() {
    ...
    return turnos.map((t) => {
      const turnoObject = t.toObject();
      const paciente = turnoObject.IdPaciente
        ? pacientes.find(
            (p) => p._id.toString() === turnoObject.IdPaciente.toString()
          )
        : null;
      const medico = turnoObject.IdMedico
        ? medicos.find(
            (m) => m._id.toString() === turnoObject.IdMedico.toString()
          )
        : null;
